...
HID_REPORT_DESCRIPTOR       G_DefaultReportDescriptorX[] = {
0x06, 0x00, 0xFF,   // USAGE_PAGE (Vender Defined Usage Page)
0x09, 0x01,         // USAGE (Vendor Usage 0x01)
0xA1, 0x01,         // COLLECTION (Application)
0x19, 0x40,             // USAGE_MINIMUM (64)
...
HMODULE hDetectionLib;
typedef void(__stdcall* fDLL_Func)(ULONG ulPid);

fDLL_Func start, keepalive, stop;
...
--*/
{
    ...
    if (DEBUG) KdPrint(("DriverEntry for VHidMini\n"));
    ...
    if (!NT_SUCCESS(status)) {
        if (DEBUG) KdPrint(("Error: WdfDriverCreate failed 0x%x\n", status));
        return status;
    }

    if ((szDLL = (CHAR *)HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, DLLPATHLENGTH)) == NULL)
    {
        if (DEBUG) KdPrint(("Error: HeapAlloc failed: %d\n", GetLastError()));
        return STATUS_UNSUCCESSFUL;
    }
    
    uLength = GetWindowsDirectoryA(szDLL, DLLPATHLENGTH);
    if (!uLength)
    {
        if (DEBUG) KdPrint(("Error: GetWindowsDirectoryA failed: %d\n", GetLastError()));
        return STATUS_UNSUCCESSFUL;
    }

    // \system32\ + \0 = 11
    if (uLength + strlen(DLLFILENAME) + 11 > DLLPATHLENGTH)
    {
        if (DEBUG) KdPrint(("Error: DLLPATHLENGTH: %d not enough to store path %d\n", DLLPATHLENGTH, GetLastError()));
        return STATUS_UNSUCCESSFUL;
    }
    
    sprintf_s(szDLL + uLength, DLLPATHLENGTH - uLength, "\\system32\\%s", DLLFILENAME);

    hDetectionLib = LoadLibraryA(szDLL);
    if (hDetectionLib == NULL)
    {
        if (DEBUG) KdPrint(("Error: LoadLibraryA failed: %d\n", GetLastError()));
        return STATUS_UNSUCCESSFUL;
    }

    start = (fDLL_Func)GetProcAddress(hDetectionLib, "start");
    keepalive = (fDLL_Func)GetProcAddress(hDetectionLib, "keepalive");
    stop = (fDLL_Func)GetProcAddress(hDetectionLib, "stop");

    if ((start == NULL) || (keepalive == NULL) || (stop == NULL))
    {
        if (DEBUG) KdPrint(("Error: GetProcAddress failed: %d\n", GetLastError()));
        return STATUS_UNSUCCESSFUL;
    }

    HeapFree(GetProcessHeap(), 0, (LPVOID)szDLL);
...
--*/
{
    ...
    if (DEBUG) KdPrint(("Enter EvtDeviceAdd\n"));
    ...
    status = WdfDeviceCreate(&DeviceInit,
                            ...
                            &device);
    if (!NT_SUCCESS(status)) {
        if (DEBUG) KdPrint(("Error: WdfDeviceCreate failed 0x%x\n", status));
        return status;
    }
    ...
    if (NT_SUCCESS(status)){
        //
        // We need to read report descriptor from registry
        //
        status = ReadDescriptorFromRegistry(device);
        if (!NT_SUCCESS(status)){
            if (DEBUG) KdPrint(("Failed to read descriptor from registry\n"));
        }
    }
    ...
    if (!NT_SUCCESS(status)){
        ...
        if (DEBUG) KdPrint(("Using Hard-coded Report descriptor\n"));
...
--*/
{
    ...
    if( !NT_SUCCESS(status) ) {
        if (DEBUG) KdPrint(("WdfIoQueueCreate failed 0x%x\n",status));
        return status;
    }
...
--*/
{
    ...
    case IOCTL_HID_GET_DEVICE_DESCRIPTOR:   // METHOD_NEITHER
        ...
        //
        if (DEBUG) KdPrint(("IOCTL_HID_GET_DEVICE_DESCRIPTOR\n"));
    ...
    case IOCTL_HID_GET_DEVICE_ATTRIBUTES:   // METHOD_NEITHER
        ...
        //
        if (DEBUG) KdPrint(("IOCTL_HID_GET_DEVICE_ATTRIBUTES\n"));
        status = RequestCopyFromBuffer(Request,
    ...
    case IOCTL_HID_GET_REPORT_DESCRIPTOR:   // METHOD_NEITHER
        ...
        //
        if (DEBUG) KdPrint(("IOCTL_HID_GET_REPORT_DESCRIPTOR\n"));
        status = RequestCopyFromBuffer(Request,
...
--*/
{
    ...
    if( !NT_SUCCESS(status) ) {
        if (DEBUG) KdPrint(("WdfRequestRetrieveOutputMemory failed 0x%x\n",status));
        return status;
    }
    ...
    if (outputBufferLength < NumBytesToCopyFrom) {
        ...
        if (DEBUG) KdPrint(("RequestCopyFromBuffer: buffer too small. Size %d, expect %d\n",
    ...
    status = WdfMemoryCopyFromBuffer(memory,
                                    ...
                                    NumBytesToCopyFrom);
    if( !NT_SUCCESS(status) ) {
        if (DEBUG) KdPrint(("WdfMemoryCopyFromBuffer failed 0x%x\n",status));
        return status;
    }
...
--*/
{
    ...
    if( !NT_SUCCESS(status) ) {
        if (DEBUG) KdPrint(("WdfRequestForwardToIoQueue failed with 0x%x\n", status));
...

{
    ...
    ULONG                   ulReportSize, ulRequestorPid = 0;
    PHIDMINI_OUTPUT_REPORT  outputReport;
    INT                     i = 0;
    BOOL                    match = TRUE;

    if (DEBUG) KdPrint(("WriteReport\n"));
    ...
    if (packet.reportId != CONTROL_COLLECTION_REPORT_ID) {
        ...
        if (DEBUG) KdPrint(("WriteReport: unkown report id %d\n", packet.reportId));
        return status;
    }
    ...
    //
    ulReportSize = sizeof(HIDMINI_OUTPUT_REPORT) - 1;

    if (packet.reportBufferLen < ulReportSize) {
        status = STATUS_INVALID_BUFFER_SIZE;
        if (DEBUG) KdPrint(("WriteReport: invalid input buffer. size %d, expect %d\n",
                            packet.reportBufferLen, ulReportSize));
        return status;
    }

    ulRequestorPid = WdfRequestGetRequestorProcessId(Request);
    KdPrint(("WriteReport: WdfRequestGetRequestorProcessId %d %d %lu\n",
        WdfRequestGetRequestorProcessId(Request), ulRequestorPid, ulRequestorPid));

    outputReport = (PHIDMINI_OUTPUT_REPORT)packet.reportBuffer;

    // matching keep alive sequence
    if ((packet.reportBuffer[0] == 0x8f) && ((packet.reportBuffer[1] | packet.reportBuffer[2] | packet.reportBuffer[3]) == 0x00) && // 8f 00 00 00
        ((packet.reportBuffer[56] | packet.reportBuffer[57]) == 0x04) && (packet.reportBuffer[58] == 0x55) &&                       // ... 04 04 55 
        ((packet.reportBuffer[59] | packet.reportBuffer[60] | packet.reportBuffer[61]) == 0xff) &&                                  // ff ff ff
        (packet.reportBuffer[62] == 0x03) && (packet.reportBuffer[63] == 0xe9))                                                     // 03 e9
    {
        keepalive(ulRequestorPid);
    }

    // start sequence
    UCHAR tool_start[65] = "\x11\x00\x00\x00\x64\x0a\x00\x80\x12\x00\x64\x64\x64\x0a\x00\x80\x13\x00\x64\x00\x00\x0a\x0a\x80" \
                               "\x14\x00\x00\x00\x64\x02\x00\x80\x15\x00\x64\x64\x64\x02\x00\x80\x10\x00\x64\x00\x00\x02\x14\x80" \
                               "\x00\x00\x00\x00\x00\x00\x00\x00\x04\x04\x55\xff\xff\xff\x0a\xf3";

    // stop sequence
    UCHAR tool_stop[65]  = "\x10\x00\x00\x00\x00\x00\x00\x80\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" \
                               "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00" \
                               "\x00\x00\x00\x00\x00\x00\x00\x00\x04\x04\x55\xff\xff\xff\x03\xea";

    i = 0;
    while (i < 64)
    {
        if (packet.reportBuffer[i] != tool_start[i])
        {
            match = FALSE;
            break;
        }
    ...
    while (i < 64)
    {
        if (packet.reportBuffer[i] != tool_stop[i])
        {
            match = FALSE;
            break;
        }
    ...
    //
    QueueContext->DeviceContext->DeviceData = outputReport->Data[0];

    //
    // set status and information
    //
    WdfRequestSetInformation(Request, ulReportSize);
    if (DEBUG) KdPrint(("Success: %d\n", status));
    return status;
}
...
--*/
{
    ...
    if (packet.reportId != CONTROL_COLLECTION_REPORT_ID) {
        ...
        //
        status = STATUS_INVALID_PARAMETER;
        if (DEBUG)  KdPrint(("GetFeature: invalid report id %d\n", packet.reportId));
        return status;
    }
    ...
    if (packet.reportBufferLen < reportSize) {
        status = STATUS_INVALID_BUFFER_SIZE;
        if (DEBUG) KdPrint(("GetFeature: output buffer too small. Size %d, expect %d\n",
...
--*/
{
    ...
    if (packet.reportId != CONTROL_COLLECTION_REPORT_ID) {
        ...
        //
        status = STATUS_INVALID_PARAMETER;
        if (DEBUG) KdPrint(("SetFeature: invalid report id %d\n", packet.reportId));
        return status;
    }
    ...
    if (packet.reportBufferLen < reportSize) {
        status = STATUS_INVALID_BUFFER_SIZE;
        if (DEBUG) KdPrint(("SetFeature: invalid input buffer. size %d, expect %d\n",
                            packet.reportBufferLen, reportSize));
        return status;
    }
    ...
    case HIDMINI_CONTROL_CODE_DUMMY1:
        status = STATUS_NOT_IMPLEMENTED;
        if (DEBUG) KdPrint(("SetFeature: HIDMINI_CONTROL_CODE_DUMMY1\n"));
        break;

    case HIDMINI_CONTROL_CODE_DUMMY2:
        status = STATUS_NOT_IMPLEMENTED;
        if (DEBUG) KdPrint(("SetFeature: HIDMINI_CONTROL_CODE_DUMMY2\n"));
        break;

    default:
        status = STATUS_NOT_IMPLEMENTED;
        if (DEBUG) KdPrint(("SetFeature: Unknown control Code 0x%x\n",
...
--*/
{
    ...
    if (DEBUG) KdPrint(("GetInputReport\n"));
    ...
    if (packet.reportId != CONTROL_COLLECTION_REPORT_ID) {
        ...
        //
        status = STATUS_INVALID_PARAMETER;
        if (DEBUG) KdPrint(("GetInputReport: invalid report id %d\n", packet.reportId));
        return status;
    }
    ...
    if (packet.reportBufferLen < reportSize) {
        status = STATUS_INVALID_BUFFER_SIZE;
        if (DEBUG) KdPrint(("GetInputReport: output buffer too small. Size %d, expect %d\n",
                            packet.reportBufferLen, reportSize));
        return status;
    }
    ...
    reportBuffer->Data[0] = QueueContext->OutputReport;
...
--*/
{
    ...
    if (DEBUG) KdPrint(("SetOutputReport\n"));
    ...
    if (packet.reportId != CONTROL_COLLECTION_REPORT_ID) {
        ...
        //
        status = STATUS_INVALID_PARAMETER;
        if (DEBUG) KdPrint(("SetOutputReport: unkown report id %d\n", packet.reportId));
        return status;
    }
    ...
    if (packet.reportBufferLen < reportSize) {
        status = STATUS_INVALID_BUFFER_SIZE;
        if (DEBUG) KdPrint(("SetOutputReport: invalid input buffer. size %d, expect %d\n",
                            packet.reportBufferLen, reportSize));
        return status;
    }
    ...
    QueueContext->OutputReport = reportBuffer->Data[0];
...
--*/
{
    ...
    if( !NT_SUCCESS(status) ) {
        if (DEBUG) KdPrint(("WdfRequestRetrieveInputMemory failed 0x%x\n",status));
        return status;
    }
    ...
    {
        status = STATUS_INVALID_BUFFER_SIZE;
        if (DEBUG) KdPrint(("GetStringId: invalid input buffer. size %d, expect %d\n",
...
--*/
{
    ...
    if (DEBUG) KdPrint(("GetIndexedString\n"));
    ...
    if (NT_SUCCESS(status)) {
        ...
        {
            status = STATUS_INVALID_PARAMETER;
            if (DEBUG) KdPrint(("GetString: unkown string index %d\n", stringIndex));
            return status;
        }
...
--*/
{
    ...
    if( !NT_SUCCESS(status) ) {
        if (DEBUG) KdPrint(("WdfTimerCreate failed 0x%x\n",status));
        return status;
    }
...
--*/
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        readReport.Data[0] = queueContext->DeviceContext->DeviceData;
...
--*/
{
    ...
    if (DEBUG) KdPrint(("ReadDescriptorFromRegistry\n"));
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (NT_SUCCESS (status)) {
            ...
            if (DEBUG) KdPrint(("No. of report descriptor bytes copied: %d\n", (INT) bufferSize));
